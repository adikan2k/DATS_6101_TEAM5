...
df_cleaned <- df_cleaned %>%
  mutate(
    first_appearance = as.Date(first_appearance, format = "%Y-%m-%d"),
    last_appearance = as.Date(last_appearance, format = "%Y-%m-%d")
  )

popularity_trend <- df_cleaned %>%
  group_by(first_appearance) %>%
  summarize(avg_duration = mean(duration_in_top50, na.rm = TRUE), .groups = "drop")

ggplot(popularity_trend, aes(x = first_appearance, y = avg_duration)) +
  geom_line(color = "blue") +
  labs(
    title = "Trend in Average Duration in Top 50 Over Time",
    x = "First Appearance Date",
    y = "Average Duration in Top 50"
  ) +
  theme_minimal()
```
...
df_cleaned <- df_cleaned %>%
  mutate(
    first_appearance = as.Date(first_appearance, format = "%Y-%m-%d"),
    month = as.integer(format(first_appearance, "%m"))
  )
...
ggplot(monthly_trend, aes(x = factor(month), y = avg_duration)) +
  geom_bar(stat = "identity", fill = "lightblue") +
  labs(
    title = "Average Duration in Top 50 by Month",
    x = "Month",
    y = "Average Duration in Top 50"
  ) +
  theme_minimal()
...
df_cleaned <- df_cleaned %>%
  ...
  summarize(
    first_appearance = min(as.Date(first_appearance)),
    last_appearance = max(as.Date(last_appearance)),
    .groups = "drop"
  ) %>%
...
ggplot(df_cleaned, aes(x = duration_in_top50)) +
  ...
  labs(
    title = "Distribution of Song Duration in the Top 50",
    x = "Duration (days)",
    y = "Count of Songs"
  ) +
  theme_minimal()
