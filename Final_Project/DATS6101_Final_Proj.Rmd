...
# Split data by class
low_class <- subset(train_data, popularity_factor == "Unpopular")
medium_class <- subset(train_data, popularity_factor == "Somewhat popular")
high_class <- subset(train_data, popularity_factor == "Highly popular")
...
set.seed(123)  # For reproducibility
subset_indices <- sample(1:nrow(X_train), size = 100000)  # Adjust size as needed
...
# Evaluate the model
library(caret)
conf_matrix <- confusionMatrix(factor(rf_predictions), factor(y_test))
print(conf_matrix)
