...
After checking for VIF, we found out that there are some multicollinearity exist in our model. Therefore, we are global_cleanedping some features.

Let's drop variables with the highest VIF stepwise, first start with `time signature` , `energy`, and `popularity` to keep VIF all under 10.
...
Lets drop the insignificant predictor `valence`.
...
__Musical Features__

Notably, we can observe a strong positive relationship between __danceability__ and higher rankings.The odds of being in the Top 25 increase by approximately 750.5%. `key`, `mode1`, and `duration_min` all have positive impact association with higher rankings.

While `speechiness`, `instrumentalness`, `acusticness`, `loudness` and `liveness` has negative association with higher rankings.

And the exponential of the coefficient of `is_explicitTRUE` is less than 1 meaning that being an explicit song (TRUE) decreases the odds of being top 25 ranking by 11.38%.

__Performance Metric__

For other performance metric, we can see that `weekly_movement` and `daily_movement` are positively associated with higher rankings.
...
global_cleaned$top_25 <- as.factor(ifelse(global_cleaned$top_25==TRUE,1,0))


conf_matrix <- confusionMatrix(data = global_cleaned$predicted, reference = global_cleaned$top_25)
print(conf_matrix)
...
global_cleaned <- global_cleaned[, -20]
str(global_cleaned)
```

__Final Remarks on Logistic Regression__

The model has a moderate accuracy (61.4%) with stronger sensitivity (64.6%) but weaker specificity (58.1%). This imbalance suggests that the model is biased toward predicting class 0.

AUC is 0.677 which is less than 0.8, and McFadden is just 0.0832 which mean only 8.32% variation in target variable is explained by the model. The model does not appear to be a good fit. That's try another modelling technique.
...
for (kval in 1:11) {
  ...
  RankCross
  # 
  cm = confusionMatrix(rank_pred, reference = test_y ) # from caret library
...
K= 2 yield the highest accuracy (0.850). To prevent overfitting, k=3 (0.848) could be a reasonable balance between high accuracy and reduced overfitting risk.
...
```{r}
loadPkg("caret") 
cmknn = confusionMatrix(rank_pred_m, reference = test_y )
print('Overall: ')
cmknn$overall
print('Class: ')
cmknn$byClass
...
The model demonstrates excellent performance with balanced metrics for both classes (Sensitivity, Specificity, and Precision are all approximately 85%).
...
cm <- confusionMatrix(predictions, test_y)
cm

```

Using cross validation k=3 still has the best performance and consistent having all total accuracy, specificty, sensitivity at around 85%.


```{r}
loadPkg("pROC")

# Ensure test_y is a factor with two levels (e.g., "0" and "1")
...
prob_positive <- ifelse(probabilities == "1", prob_positive, 1 - prob_positive)
...
AUC (0.913) > 0.8 indicates the model is a good fit.
...
K= 7 yield the highest accuracy (0.747).
...
The best model with k =3 demonstrates strong performance with an accuracy of 75.3% and balanced sensitivity (72.6%) and specificity (77.9%). While the model is slightly better at identifying class 1 than class 0, it provides a good overall balance


Using cross validation and test set approaches give me different best k, using cross validation knn has the best performance when k=3 and using test set knn has best accuracy when k=7.
...
auc_value <- auc(roc_curve)
cat("AUC:", auc_value, "\n")
plot(roc_curve, main = "KNN ROC Curve", col = "blue", lwd = 2)
