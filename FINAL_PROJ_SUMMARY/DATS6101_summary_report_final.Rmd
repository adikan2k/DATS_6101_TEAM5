...
To address this question, our analysis began with comprehensive Exploratory Data Analysis (EDA) to uncover the relationships between key musical attributes and song popularity. The dataset, comprising 1.4 million records of Spotify's top tracks across 70+ countries, revealed varying distributions for features like loudness, energy, and danceability. These initial findings set the stage for model selection and refinement.


We first explored Linear Regression as a baseline model to predict popularity, treating the target as a continuous variable. However, this model performed poorly, with residual plots revealing heteroscedasticity and non-normal distributions. This highlighted that linear models were insufficient for capturing the complex, non-linear relationships between features like loudness, energy, and acousticness.

The target variable, song popularity, was categorized into low, medium, and high classes, with SMOTE oversampling applied to balance the dataset. Logistic Regression was then employed to these categories. Although it improved interpretability, multicollinearity among features (e.g., energy and loudness) and an inability to distinguish medium and high classes resulted in underwhelming performance.

Ordinal Logistic Regression was considered to account for the ordinal nature of the target variable. While this model addressed some challenges, the proportional odds assumption was violated, confirmed by the Brant test. The model achieved moderate accuracy (~52%) but failed to produce reliable results for the low popularity class, reflecting its inability to handle the dataset’s inherent complexities. 


Ultimately, The Random Forest model achieved a robust accuracy of 93.33%, with precision, recall, and F1 scores all exceeding 93%, demonstrating high reliability. As illustrated in the feature importance plot below, loudness, energy, and acousticness emerged as the most influential predictors of song popularity. These findings align with real-world observations, where songs with higher loudness and danceability frequently rank as global hits. The model's predictions, such as identifying likely hit songs based on these attributes, closely match industry trends and offer actionable insights for producers and marketers.
...
### 4. Time Series Decomposition of Song Popularity Trends

In this analysis, we explored the temporal trends in song popularity using time series decomposition. The objective was to break down the song popularity data into its core components—trend, seasonality, and residuals—to uncover the underlying patterns that influence a song's performance over time. The dataset used for this analysis, top_songs, contains song names and their aggregated popularity metrics. This method allows us to differentiate between long-term changes, recurring patterns, and random fluctuations in popularity.
...
Observed: The top panel displays the raw data, illustrating the overall patterns and fluctuations in song popularity. This includes a mix of trend, seasonality, and noise.
Trend: The second panel isolates the long-term trend in song popularity. It highlights gradual increases or decreases over time, reflecting broader shifts in the music industry or listener preferences.
Seasonality: The third panel shows recurring patterns, revealing periodic rises and falls in popularity. These may correspond to weekly or monthly cycles influenced by events such as album releases or seasonal festivities.
Residual: The bottom panel displays the random fluctuations remaining after removing the trend and seasonality. These irregularities capture unpredictable changes, such as sudden spikes due to viral popularity or unexpected dips.

# The three advanced statistical analyses
...
# Clustering Popularity Trajectories
